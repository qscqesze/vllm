...
"""Inference-only MiniMaxVL01 model."""
import copy
import math
import re
...
class MiniMaxVL01RMSNormTP(CustomOp):
    name = "MiniMaxVL01RMSNormTP"
...
class MiniMaxVL01RotaryEmbedding(CustomOp):
    name = "MiniMaxVL01RotaryEmbedding"
...
class MiniMaxVL01MLP(nn.Module):
...
class MiniMaxVL01MoE(nn.Module):
    ...
    def __init__(
        ...
        self.gate.weight.weight_loader = MiniMaxVL01MoE.gate_weight_loader
...
class MiniMaxVL01LinearAttention(nn.Module):
    ...
    def __init__(
        ...
        self.norm = MiniMaxVL01RMSNormTP(
            self.hidden_inner_size,
            eps=1e-5,
        )

        slope_rate = MiniMaxVL01LinearAttention._build_slope_tensor(
            self.num_heads)
    ...
    def _prefill_and_mix_infer(self, q, k, v, kv_cache, state_indices_tensor,
        ...
        for _prefill_idx in range(getattr(attn_metadata, "num_prefills", 0)):
            ...
            out_slice = MiniMaxVL01LinearKernel.jit_linear_forward_prefix(
                qs,
                ks,
                vs,
                slice_layer_cache,
...
class MiniMaxVL01Attention(nn.Module):
...
class MiniMaxVL01DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if config.attention_type == 0:
            ...
            self.self_attn = MiniMaxVL01LinearAttention(
                hidden_size=self.hidden_size,
            ...
            self.self_attn = MiniMaxVL01Attention(
                hidden_size=self.hidden_size,
                num_heads=config.num_attention_heads,
                head_dim=head_dim,
        ...
        if expert_num == 1:
            ...
            self.block_sparse_moe = MiniMaxVL01MoE(
...
class MiniMaxVL01Model(nn.Module):
    ...
    def __init__(
        ...
        def layer_fn(prefix):
            ...
            return MiniMaxVL01DecoderLayer(layer_config,
                                             **decoder_kwargs,
                                             prefix=prefix)
        ...
        if hasattr(config, "max_model_len") and isinstance(
            ...
            max_position_embeddings = min(config.max_position_embeddings,
                                          config.max_model_len)
        self.rotary_emb = MiniMaxVL01RotaryEmbedding(
            head_dim,
            rotary_dim=config.rotary_dim
            if hasattr(config, "rotary_dim") else head_dim,
            max_position=max_position_embeddings,
    ...
    def load_weights(self, weights: Iterable[Tuple[str,
        ...
        def load_linear_attn_weight(name: str, loaded_weight: torch.Tensor,
            ...
            weight_loader = getattr(
                param, "weight_loader",
                MiniMaxVL01LinearAttention.weight_direct_load)
            weight_loader = weight_loader_with_alias(name)(weight_loader)
            weight_loader(param, loaded_weight)
            return
